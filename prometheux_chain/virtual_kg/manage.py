...
def cleanup_kg(virtual_kg_id=None, scope="user"):
    """
    Cleanup Virtual KG resources for the user.
    If `virtual_kg_id` is provided, it only cleans up resources for that KG.
    If `virtual_kg_id` is None, it cleans up all resources for the user associated with the PMTX token.
    """
    if virtual_kg_id is not None:
        delete_virtual_kg_resource_response = JarvisPyClient.cleanup(virtual_kg_id, scope)
    else:
...
def list_kgs(scopes=["user", "organization"]):
    """
...
def load_kg(kg_id, scope="user"):
    """
    Load a knowledge graph by its ID.
    """
    response = JarvisPyClient.load_kg(kg_id, scope)
...
def save_kg_chat(prompt, response_text, kg_id, scope="user"):
    """
    Save a chat interaction for a specific knowledge graph.
    Returns the chat_id of the saved interaction.
    """
    response = JarvisPyClient.save_kg_chat(kg_id, scope, prompt, response_text)
...
def load_kg_chat(kg_id, scope="user"):
    """
    Load chat interactions for a specific knowledge graph by its ID.
    """
    response = JarvisPyClient.load_kg_chat(kg_id, scope)
...
def cleanup_kg_chat(kg_id, chat_ids=None, scope="user"):
    """
    ...
    """
    response = JarvisPyClient.cleanup_kg_chat(kg_id, scope, chat_ids)
...
def save_kg_query(kg_id, query, scope="user"):
    """
    Save a query for a specific knowledge graph.
    """
    response = JarvisPyClient.save_kg_query(kg_id, scope, query)
...
def load_kg_queries(kg_id, scope="user"):
    """
    Load all queries for a specific knowledge graph.
    """
    response = JarvisPyClient.load_kg_queries(kg_id, scope)

    # Handle response
    if response.get('status') != 'success':
        msg = response.get('message', 'Unknown error')
        raise Exception(f"An exception occurred while loading queries: {msg}")
    
    if response.get('status') == 'success':
        return response.get('data', [])
    else:
        raise Exception(f"An exception occurred while loading queries: {response.get('message', 'Unknown error')}")


def cleanup_kg_queries(kg_id, query_ids=None, scope="user"):
    """
    Cleanup queries for a specific knowledge graph.
    If `query_ids` is provided, it only cleans up those specific queries.
    If `query_ids` is None, it cleans up all queries for the knowledge graph.
    """
    response = JarvisPyClient.cleanup_kg_queries(kg_id, scope, query_ids)
