...
def list_policies(ontology_id, scope="user", concept_name=None):
    ...
    return _check(JarvisPyClient.list_policies(
        ontology_id=ontology_id, scope=scope, concept_name=concept_name,
    ), "list policies")
...
def get_policy(ontology_id, policy_id, scope="user"):
    ...
    return _check(JarvisPyClient.get_policy(
        ontology_id=ontology_id, policy_id=policy_id, scope=scope,
    ), "get policy")
...
def get_run_history(ontology_id, policy_id, scope="user", limit=50, offset=0):
    ...
    return _check(JarvisPyClient.get_run_history(
        ontology_id=ontology_id, policy_id=policy_id, scope=scope, limit=limit, offset=offset,
    ), "run history")
