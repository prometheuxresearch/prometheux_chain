...
def create_policy(project_id, concept_name, trigger_type="cron", trigger_config=None,
    ...
    return _check(JarvisPyClient.create_policy(
        project_id=project_id, concept_name=concept_name, trigger_type=trigger_type,
        trigger_config=trigger_config, scope=scope, enabled=enabled,
    ), "create policy")
...
def list_policies(project_id, scope="user", concept_name=None):
    ...
    return _check(JarvisPyClient.list_policies(
        project_id=project_id, scope=scope, concept_name=concept_name,
    ), "list policies")
...
def get_policy(project_id, policy_id, scope="user"):
    ...
    return _check(JarvisPyClient.get_policy(
        project_id=project_id, policy_id=policy_id, scope=scope,
    ), "get policy")
...
def get_run_history(project_id, policy_id, scope="user", limit=50, offset=0):
    ...
    return _check(JarvisPyClient.get_run_history(
        project_id=project_id, policy_id=policy_id, scope=scope, limit=limit, offset=offset,
    ), "run history")
